...
namespace LoU
{
    ...
    internal static class Utils
    {
        ...
        public static List<MobileInstance> FindMobile(string name, float distance = 50)
        {
            ...
            {
                if (mobile.transform.position.DistanceFrom(playerPosition) > distance) break;
    ...
    internal static class Logging
    {
        ...
        private static void LogProps(object obj)
        {
            ...
            foreach (PropertyDescriptor descriptor in TypeDescriptor.GetProperties(obj))
            {
                ...
                foreach (string key in collection) Log($"Key:{name}.{key} = Value:{name}.{((IDictionary) value)[key]}");
            }
